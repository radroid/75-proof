...
| # | Item | Priority | Status | Notes |
|---|------|----------|--------|-------|
| C-1 | Persistent coach memory file | P1 | todo | **Goal:** The coach should know each user — goals, schedule constraints, what's worked, what hasn't — across sessions. **Implementation:** Maintain a small structured memory document per user (JSON or `.md`, CLAUDE.md-style: short, lossy by design). Store in `users.coachMemory` for authed accounts and in the local store for guests. Update after each meaningful exchange via a "memory writer" prompt that distills the chat down to durable facts. Inject the latest snapshot into every coach prompt. Cap at ~2KB so the context overhead stays small. |
| C-2 | Coach chat history & threads | P1 | todo | Persist all coach turns (not just the live session). Show a left-rail of past threads on desktop / bottom-sheet on mobile. The first onboarding "AI path" conversation must auto-promote into a thread so its context is preserved. New threads inherit the user's coach memory but not prior thread transcripts (keep context windows small). |
...
| C-5 | Downloadable coach context (privacy) | P1 | todo | One-click export of the coach memory + thread history as a portable JSON/Markdown bundle. Include a short README explaining the schema so users can carry context to ChatGPT/Claude/etc. Reinforces the privacy-first stance: their data, their context, their move. Pairs naturally with P-1 (data export). |
