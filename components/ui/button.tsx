  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all active:scale-[0.98] disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
  {
    variants: {
      variant: {
        ...
        // Outline + ghost variants use the secondary surface for hover —
        // not accent. `--accent` is the brand emphasis color across themes
        // (loud blue/red/tan/sand) and `--accent-foreground` is tuned for
        // contrast against THAT color, not against the at-rest button bg.
        // Using secondary keeps text-foreground readable through the whole
        // state transition.
        outline:
          "border bg-background text-foreground shadow-xs hover:bg-secondary dark:bg-input/30 dark:border-input dark:hover:bg-input/50",
        secondary:
          "bg-secondary text-secondary-foreground shadow-sm hover:bg-secondary/80",
        ghost:
          "text-foreground hover:bg-secondary dark:hover:bg-secondary/60",
        link: "text-primary underline-offset-4 hover:underline",
        gradient:
          "bg-[var(--gradient-primary)] text-primary-foreground shadow-md hover:shadow-lg hover:brightness-105",
        glow: "bg-primary text-primary-foreground shadow-sm hover:shadow-[var(--glow-primary)]",
        success:
