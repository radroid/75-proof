...
| State                                                           | Treatment                                                                                                                                                                           |
| --------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Pre-start (`startDate` is in the future)                        | Both `/dashboard` and `/dashboard/progress` route to `<ChallengeUpcoming>` instead of the stat grid. It renders a countdown that tightens as the day approaches ("Starts in 5 days" → "Starts day after tomorrow" → "Starts tomorrow") plus a reminder that Settings can still change the start date / routine. No streak, no rolling-rate, no Today snapshot — none of which have a denominator yet. |
