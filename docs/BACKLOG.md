...
| PD-12 | Dedicated `getRecentReactionsForMe` Convex query | P3 | todo | Promote the v1 client-side compose (over `feedReactions` + `activityFeed`) to a server query if telemetry shows it's hit on every Progress page load. Pure perf optimization. |
| PD-13 | Dual-mode lifetime-vs-active scoping toggle | P3 | todo | Explicit headline-vs-history scope toggle for users who switch between fixed challenges and habit-tracker mode. Only ship if PD-1's "active challenge only" rule causes confusion in telemetry. |
| PD-14 | Friends merge — Phase 3 (collapse bottom-nav slot) | P3 | todo | Once Phase 1 (ribbon) + Phase 2 (Activity tab) bake in, collapse the standalone Friends bottom-nav slot into a Settings-style management surface reachable from Progress. Wait for usage data on the merged page first. |
...
| H-3 | ~~LLM-assisted habit creation~~ | | done | Full-screen AI coach onboarding shipped in PR #18. Three-path picker (AI / Browse popular / Custom), `OnboardingPersonalizeChat` with sentinel-tagged proposal parser, Convex `personalize.chat` action via OpenRouter, returns to Review on Back. AI path now first-class — see `OnboardingPathSelect.tsx`, `OnboardingAiStep.tsx`, `convex/personalize.ts`. |
| H-4 | ~~Challenge templates library~~ | | done | `popularRoutines` Convex table seeded from `convex/lib/popularRoutinesSeed.ts` (4 categories: fitness, skill-building, productivity, personal-development), `OnboardingBrowsePopular.tsx` step renders the catalog with category filters, vector-search action backs the coach's "find me a routine like X" query. Static `lib/routine-templates.ts` retained for known catalog templates (75 HARD, 30-Day Yoga). Full curated catalog landed via PR #14 + #16. |
| H-5 | Habit cadence — weekly / monthly / yearly targets, not just daily | P1 | todo | **Problem:** Today every habit is "do this every day"; missing a hard one resets the challenge. Many real routines are 3-5×/week, weekly long-runs, monthly reviews, or yearly counts (e.g. "run 1000 miles this year"). The popular catalog has these intents in the duration string but we collapse them all to daily checkboxes. **Solution:** Move from "isHard daily" → habit cadence (`daily` / `weekly:N` / `monthly:N` / `yearly:N`) + completion model (`hard`: must hit target or fail; `soft`: target is a goal, partials count). Entries roll up to the relevant period; the dashboard shows period-progress, not just today. **Migration:** old habits map to `cadence: "daily", completion: isHard ? "hard" : "soft"`. Schema: add `cadence`, `targetCount` to habitDefinitions; add a period rollup query. UI: per-habit cadence picker in onboarding, period-aware progress on Today, and a weekly view that surfaces the upcoming due-by-Sunday targets. |
| H-6 | Drag-to-navigate onboarding step dots + haptics | P3 | todo | The current StepIndicator dots are clickable for visited steps. On touch devices, also support a horizontal drag gesture across the dot row to scrub between steps with light haptic ticks per dot crossed. Desktop: arrow keys jump between visited dots. |
...
| S-4 | Milestone celebrations | P2 | in-progress | Backend emits milestone events to the activity feed (Day 7/14/21/30/45/60/75 in `convex/challenges.ts`) and friends already see them. **Remaining:** in-app celebration UI (confetti, full-screen badge moment), opt-in push notifications to the milestone-hitter, and a friend-side reaction prompt ("send a cheer"). |
...
> The Routine Coach is becoming the central interaction surface — onboarding helper, daily check-in companion, and notification author. These items shape that arc.

| # | Item | Priority | Status | Notes |
|---|------|----------|--------|-------|
| C-1 | ~~Persistent coach memory file~~ | | done | Shipped in PR #20. `users.coachMemory` blob distilled by `convex/coachActions.ts` (memory writer runs via `after()` post-response), injected into every chat turn from `app/api/coach/chat/route.ts`. Privacy controls in `CoachPrivacySettings.tsx`: opt-in toggle, one-tap purge, audit log surfaced to the user. Cascade-on-delete wired into account deletion. |
| C-2 | ~~Coach chat history & threads~~ | | done | Shipped in PR #20. Threads persisted in Convex with `CoachThreadList.tsx` rail; new threads inherit memory, not transcripts. Onboarding AI conversation auto-promotes into the user's first thread. Per-thread delete + "forget me" wipe-all in `CoachPrivacySettings.tsx`. |
| C-3 | Coach as central comms hub | P1 | todo | Replace generic system notifications/emails with coach-authored messages: daily reminders, milestone celebrations (pairs with S-4 UI), missed-habit nudges, restart commiseration. Tone set by user's theme + coach memory. Backend: a `coachMessage` action that the existing notification scheduler calls instead of sending raw strings. **Bumped to P1** now that C-1/C-2/C-5 unlocked the memory + threads substrate this depends on. |
| C-4 | Two-way coach chat — actions on routine | P1 | todo | Chat plumbing exists (CoachClient.tsx already accepts user-initiated messages anytime). What's missing is the *action* layer: a small allowlist of mutations the coach can propose — pause a habit, swap one out, schedule a rest day, change a target — surfaced as inline confirm chips in the reply ("Pause leg day? [Confirm] [No]"). Gated behind explicit user confirm; logged to audit. **Bumped to P1** because it converts the coach from a Q&A surface into the editing surface for routines, which unlocks H-2 (edit habits post-onboarding) without a separate settings page. |
| C-5 | ~~Downloadable coach context (privacy)~~ | | done | Shipped in PR #20. `/api/coach/export` route returns memory + thread bundle as JSON; export event logged to the audit trail. Reachable from `CoachPrivacySettings.tsx`. Pairs with P-1 when account-wide export ships. |
| C-6 | Coach feedback loop into routine quality | P3 | todo | Each chat exchange should produce a tiny per-routine signal: "user adopted", "user abandoned after N days", "user said it was too hard". Aggregate these into the popular routines catalog so future coach recommendations get smarter without sending PII off-device. |
| C-7 | Coach can reset & reconfigure the active challenge | P2 | todo | The coach should be able to translate a chat conversation into a new challenge configuration — e.g. "swap one workout for outdoor sport, layer in a shutdown ritual, add weekly social". Surface as a confirm step in the reply ("Apply this 3-week ramp? [Confirm] [No]"), then call a `coach:applyChallengePlan` mutation. **Depends on C-4 (action layer) and on a varying-routine tracker that supports week-by-week phase changes** — the current habit tracker assumes a single fixed habit set for the whole challenge, which can't represent the "Week 1–2 / Week 3 / Week 4+" shape coaches actually prescribe. Builds the foundation for plan-driven challenges instead of fixed-template challenges. |
...
| ~~H-3~~ | LLM-assisted habit creation (full-screen AI coach onboarding, 3-path picker) | 2026-04 (PR #18) |
| ~~H-4~~ | Challenge templates library (popular routines catalog + Browse Popular onboarding step) | 2026-04 (PR #14, #16) |
