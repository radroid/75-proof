...
| # | Item | Priority | Status | Notes |
|---|------|----------|--------|-------|
| C-1 | Persistent coach memory file | P1 | todo | **Goal:** The coach should know each user — goals, schedule constraints, what's worked, what hasn't — across sessions. **Implementation:** Maintain a small structured memory document per user (JSON or `.md`, CLAUDE.md-style: short, lossy by design). Store in `users.coachMemory` for authed accounts and in the local store for guests. Update after each meaningful exchange via a "memory writer" prompt that distills the chat down to durable facts. Inject the latest snapshot into every coach prompt. Cap at ~2KB so the context overhead stays small. **Privacy/retention (must ship together):** opt-in (default off for authed; local-mode is per-device anyway), one-tap "forget me" that purges memory + threads in a single mutation, default 90-day TTL with a per-user opt-out, and an explicit redaction allowlist (no display name, email, or free-text identifiers in the persisted blob). Log every memory write/purge (manual or TTL) to an audit trail the user can read; cascade memory deletion through P-2 account deletion. |
| C-2 | Coach chat history & threads | P1 | todo | Persist all coach turns (not just the live session). Show a left-rail of past threads on desktop / bottom-sheet on mobile. The first onboarding "AI path" conversation must auto-promote into a thread so its context is preserved. New threads inherit the user's coach memory but not prior thread transcripts (keep context windows small). **Privacy/retention:** same TTL + manual-purge contract as C-1 (default 90 days, per-thread "delete" button, "forget me" wipes all threads). Cascade through P-2 account deletion. |
| C-3 | Coach as central comms hub | P2 | todo | Replace generic system notifications/emails with coach-authored messages: daily reminders, milestone celebrations, missed-habit nudges, restart commiseration. Tone is set by the user's chosen theme + memory. Backend: a `coachMessage` action that the existing notification scheduler calls instead of sending raw strings. |
| C-4 | Two-way coach chat (user-initiated, anytime) | P2 | todo | The coach surface should always accept a message ("How am I doing this week?", "Switch leg day to Wednesday"). Replies should be able to *act* on the routine — pause a habit, swap one out, schedule a rest day — through a small allowlist of mutations gated by an explicit user-confirm step. |
| C-5 | Downloadable coach context (privacy) | P1 | todo | One-click export of the coach memory + thread history as a portable JSON/Markdown bundle. Include a short README explaining the schema so users can carry context to ChatGPT/Claude/etc. Reinforces the privacy-first stance: their data, their context, their move. Pairs naturally with P-1 (data export). **Audit:** include the C-1/C-2 audit log in the bundle so users see exactly what was stored, when it was written, and when it would have been purged. The export action itself counts as a consent event and gets recorded. |
